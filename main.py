...
def main():
    ...
    while True:
        ...
        table.add_row("[1]", "🎲 Generate & Check (pattern)")
        table.add_row("[2]", "📖 Dictionary Check (wordlist)")
        table.add_row("[3]", "⚙️  Settings")
        table.add_row("[4]", "🚪 Exit")
        
        console.print(Panel(table, title="[bold]Main Menu[/]", border_style="cyan"))
        
        choice = Prompt.ask("  ▶  Enter your choice", choices=["1", "2", "3", "4"], default="1")
        
        if choice == "1":
            menu_generate_and_check()
        elif choice == "2":
            menu_dictionary_check()
        elif choice == "3":
            menu_settings()
        elif choice == "4":
            console.print("\n  [bold cyan]Goodbye![/]\n")
